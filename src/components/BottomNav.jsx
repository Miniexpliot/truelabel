...
const BottomNav = ({ activeTab, setActiveTab, onCapture, show }) => {
  ...
  return (
    <div className={`fixed left-4 right-4 bg-white/80 backdrop-blur-xl border border-white/40 shadow-lg rounded-3xl z-50 px-2 py-2 block sm:hidden transition-all duration-500 ease-out ${
      show 
        ? 'bottom-6 opacity-100 translate-y-0' 
        : '-bottom-24 opacity-0 translate-y-8 pointer-events-none'
    }`}>
